...
def importBddParser(subparsers):
  parser = subparsers.add_parser('importBdd',
    description='Import BDD annotations into a db (the format after 08-28-2018). '
    'Both image-level and object-level attributes are written to the "properties" table. '
    '"manualShape" and "manualAttributes" are ignored. '
  ...
  for image_path in progressbar(image_paths):
    ...
    if args.detection_json:
      ...
      for object_ in detections_for_image['labels']:
        ...
        c.execute('INSERT INTO objects(imagefile,x1,y1,width,height,name) '
              'VALUES (?,?,?,?,?,?)', (imagefile,x1,y1,width,height,object_name))
        objectid = c.lastrowid
        ...
        if 'poly2d' in object_:
          ...
          for pt in polygon['vertices']:
            c.execute('INSERT INTO polygons(objectid,x,y,name) '
                       'VALUES (?,?,?,?)', (objectid,pt[0],pt[1],polygon_name))
          if args.with_display:
            drawScoredPolygon (img, [(int(x[0]), int(x[1])) for x in polygon['vertices']], object_name)

        # Insert image-level and object-level attributes to "properties" table.
    ...
    if args.segmentation_dir:
      segmentation_path = op.join(args.segmentation_dir, '%s_train_id.png' % filename)
