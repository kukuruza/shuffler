  ...
  for image_path in progressbar(image_paths):
    ...
    if args.annotations:
      ...
      for object_ in tree.getroot().findall('object'):
        ...
        pose      = object_.find('pose').text      if object_.findall('pose') else None
        truncated = object_.find('truncated').text if object_.findall('truncated') else None
        difficult = object_.find('difficult').text if object_.findall('difficult') else None
        occluded  = object_.find('occluded').text  if object_.findall('occluded') else None
        x1        = int(float(object_.find('bndbox').find('xmin').text))
        y1        = int(float(object_.find('bndbox').find('ymin').text))
        width     = int(float(object_.find('bndbox').find('xmax').text)) - x1 + 1
        height    = int(float(object_.find('bndbox').find('ymax').text)) - y1 + 1
        ...
        for part in object_.findall('part'):
          # Each part in inserted as a 4-point polygon with a name of the part.
