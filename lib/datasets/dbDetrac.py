...
def importDetracParser(subparsers):
  parser = subparsers.add_parser('importDetrac',
    description='Import DETRAC annotations into a db. ')
  ...
  for video_dir in progressbar(video_dirs):
    ...
    for image_path in image_paths:
      ...
      if not op.exists(image_path):
        raise FileNotFoundError('Image file not found at "%s".' % image_path)

      imheight, imwidth = getPictureSize(image_path)
      imagefile = op.relpath(image_path, args.rootdir)
      logging.debug('Parsed image into imagefile=%s, width=%s, height=%s' % (imagefile,imwidth,imheight))
      c.execute('INSERT INTO images(imagefile,width,height) VALUES (?,?,?)', (imagefile,imwidth,imheight))
    ...
    if args.tracking_dir is not None:
      ...
      for frame in tree.getroot().findall('frame'):
        ...
        if not image_path in image_paths:
          raise Exception('Imagepath %s the xml %s is not a jpeg in dir: %s' %
        ...
        for target in frame.find('target_list').findall('target'):
          ...
          logging.debug ('Parsed box: %s' % str([x1, y1, width, height]))
          c.execute('INSERT INTO objects(imagefile,x1,y1,width,height) VALUES (?,?,?,?,?)',
          ...
          c.execute('INSERT INTO matches(objectid,match) VALUES (?,?)', (objectid,match))

          # Parse properties.
          attribute = target.find('attribute')
          orientation = float(attribute.attrib['orientation'])
          speed = float(attribute.attrib['speed'])
          trajectory_length = int(attribute.attrib['trajectory_length'])
          vehicle_type = attribute.attrib['vehicle_type']
          c.execute('INSERT INTO properties(objectid,key,value) VALUES (?,?,?)', (objectid,'orientation',str(orientation)))
          c.execute('INSERT INTO properties(objectid,key,value) VALUES (?,?,?)', (objectid,'speed',str(speed)))
          c.execute('INSERT INTO properties(objectid,key,value) VALUES (?,?,?)', (objectid,'vehicle_type',vehicle_type))
          c.execute('INSERT INTO properties(objectid,key,value) VALUES (?,?,?)', (objectid,'trajectory_length',str(trajectory_length)))
          
          if args.with_display:
            drawScoredRoi (img, bbox2roi([x1,y1,width,height]), vehicle_type)
