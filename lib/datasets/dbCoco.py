...
def exportCocoParser(subparsers):
  parser = subparsers.add_parser('exportCoco',
  ...
  images_policy.add_argument('--copy_images', action='store_true',
      help='If specified, will copy images to args.coco_dir/"images"/args.subset. '
      'Required, if the media is stored as video. ')
  images_policy.add_argument('--symlink_images', action='store_true',
  ...
  images_policy.add_argument('--symlink_image_folder', action='store_true',
  ...
  parser.add_argument('--categories', nargs='+',
      help='If specified, will write only object names in the "categories" list. '
  ...
  parser.add_argument('--license_url',
      help='Optional license url, e.g. "http://creativecommons.org/licenses/by/2.0/"')
...
def exportCoco (c, args):
  ...
  for imageid, image_entry in progressbar(enumerate(c.fetchall())):
    ...
    images.append({
      "id" : imageid, "width" : width, "height" : height, "file_name" : file_name,
    ...
    if not op.exists(old_image_path):
      raise FileNotFoundError("Image not found at %s. (Using rootdir %s.)" %
    ...
    if args.copy_images:
      ...
      os.symlink(op.abspath(old_image_dir), new_image_dir, target_is_directory=True)
  ...
  for icategory, category in enumerate(categories):
    ...
    categories_coco.append({"supercategory": None, "id": icategory, "name": category})
  ...
  for object_entry in progressbar(c.fetchall()):
    ...
    annotations.append({
      ...
      "segmentation" : polygons_coco, "area" : area, "bbox" : list(bbox), 
      "iscrowd" : 0,
    })
  ...
  json_path = op.join(args.coco_dir, 'annotations', 'instances_%s.json' % args.subset)
