...
from lib.backendDb import objectField #, polygonField, deleteImage, parseTimeString, makeTimeString
...
def labelAzimuth (c, args):
  ...
  while button != 27:
    ...
    if another_object:
      ...
      c.execute('SELECT value FROM properties WHERE objectid=? AND key="yaw"', (objectid,))
      yaw = c.fetchone()
      ...
      try:
        ...
        # A hack that allowed me write all the images into one dir but keep info about imagefile.
        c.execute('SELECT name FROM images WHERE imagefile=?', (imagefile,))
        image_name = c.fetchone()[0]
        flattening = _getFlatteningFromImagefile(poses, image_name, y, x)
    ...
    if action == 'delete':
      c.execute('DELETE FROM properties WHERE objectid=? AND key="yaw"', (objectid,))
    ...
    # Entry happened one way or the other. Update the yaw and go to the next object.
    if update_yaw_in_db:
      c.execute('SELECT COUNT(1) FROM properties WHERE objectid=? AND key="yaw"', (objectid,))
      if c.fetchone()[0] == 0:
        c.execute('INSERT INTO properties(value,key,objectid) VALUES (?,"yaw",?)', (str(yaw),objectid))
      else:
        c.execute('UPDATE properties SET value=? WHERE objectid=? AND key="yaw"', (str(yaw),objectid))
