...
from lib.util import applyLabelMappingToMask
from lib.dbEvaluate import *
...
def evaluateSegmentationOneImage(args):
  ...
  cv2.imshow('mask_gt', cv2.resize(mask_gt, (512, 512), interpolation=cv2.INTER_NEAREST))
  cv2.imshow('mask_pr', cv2.resize(mask_pr, (512, 512), interpolation=cv2.INTER_NEAREST))
  cv2.waitKey(-1)
