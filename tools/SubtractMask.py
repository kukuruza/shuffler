...
def subtractMask_parser():
  parser = ArgumentParser('Take a difference between masks')
...
def subtractMask(args):
  ...
  logging.info('%d frames in video %s.' % (imreader1.get_length(), args.in_mask_path1))
  logging.info('%d frames in video %s.' % (imreader2.get_length(), args.in_mask_path2))
  if not args.dryrun:
    imwriter = VideoWriter(vmaskfile=args.out_mask_path, overwrite=args.overwrite)
