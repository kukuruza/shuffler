...
def erodeMask_parser():
  parser = ArgumentParser('Erode one color in a mask, in favor or another color.')
...
def erodeMask(args):
  ...
  for i in progressbar.progressbar(range(imreader.get_length())):
    ...
    roieroded = cv2.erode(roi.astype(np.uint8), kernel=np.ones((3,3)),
      iterations=args.depth, borderValue=0).astype(bool)
