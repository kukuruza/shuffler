...
class DatasetWriter:
  ...
  def addImage(self, image_dict):
    ''' Records an image to database and maybe to disk.
    Args:
      image_dict:   A dict that should have some of the following keys:
                      * "image", "imagefile" (either of the two is required.)

                        "image" is a numpy array standing for an image.
                        It will be recorded on disk as a file or a video frame,
                        based on "media" argument of the constructor.
                        It will also be recorded as an entry to "images" table.
                        "imagefile" is a path string corresponding to an image
                        on disk. When specified, that image on disk will be
                        recorded as an entry to the "images" table.

                      * "mask" or "maskfile" (either of the two, or omitted.)

                        Same behavior as for "image" or "imagefile".
                        When ommited, no maskfile is written to "images" table.

                      * "timestamp" is an object of datetime.datetime type,
                        defines the time the image was created. If ommited,
    ...
    if not isinstance(image_dict, Mapping):
      raise TypeError('image_dict should be a dict, not %s' % type(object_dict))
      ...
      if self.media == 'pictures':
        ...
        if not op.exists(imagepath):
          raise ValueError('Got imagefile "%s" with rootdir "%s", '
            'but cant find an image at "%s"' % (imagefile, self.rootdir, imagepath))
    ...
    if 'mask' in image_dict and 'maskfile' in image_dict:
      raise ValueError('Only one of "mask" or "maskfile" can be specified.')
      ...
      if self.media == 'pictures':
        ...
        if not op.exists(maskpath):
          raise ValueError('Got maskfile "%s" with rootdir "%s", '
            'but cant find an image at "%s"' % (maskfile, self.rootdir, maskpath))
      # Check video on disk, and get the dimensions from user's input.
      elif self.media == 'video':
    ...
    if 'timestamp' in image_dict:
      ...
      if not isinstance(timestamp, datetime):
        raise TypeError('Timestamp should be an object of datetime.datetime class. '
    ...
    image_entry = (imagefile,width,height,maskfile,timestamp,name,score)
...
if __name__ == "__main__":
  ...
  ' Demo of how to use DatasetWriter. Writes images and objects with some dummy data. '

  parser = argparse.ArgumentParser()
  parser.add_argument('-o', '--out_db_file', required=True)
  parser.add_argument('--media', choices=['pictures', 'video'], default='pictures',
      help='Whether to write images as JPG/PNG files or as frames in a video.')
  parser.add_argument('--image_path',
      help='If media is "pictures", pass the path to the new folder with pictures. '
      'If media is "video", pass the path to that video.')
  parser.add_argument('--mask_path',
      help='If media is "pictures", pass the path to the new folder with pictures. '
      'If media is "video", pass the path to that video.')
  parser.add_argument('--rootdir', default='.',
      help='Directory that "imagefile" and "maskfile" entries will be relative to.')
  ...
  with DatasetWriter(
    ...
    imagefile1 = writer.addImage({
        ...
        'mask': np.random.randint(128, size=(100,100), dtype=np.uint8),
    })
    ...
    objectid1 = writer.addObject({
        ...
        'x1': 10, 'y1': 20, 'width': 30, 'height': 40,
        'name': 'myobject',
        'fancy color property': 'green',
    })
    objectid2 = writer.addObject({
        'imagefile': imagefile2,
        'x1': 40, 'y1': 30, 'width': 20, 'height': 10,
        'name': 'myobject',
    })
