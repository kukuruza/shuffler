...
class TestDatasetWriter (unittest.TestCase):
  ...
  if os.name == 'nt':
    WORK_DIR = op.join(op.dirname(op.realpath(__file__)), 'tmp/TestDatasetWriter')
  else:
    WORK_DIR = '/tmp/TestDatasetWriter'
  ...
  def test_record_imagevideo (self):
    ...
    self.writer = DatasetWriter(out_db_file, image_path=image_path, media='video')
    self.writer.addImage({'image': image})
    self.writer.addImage({'image': image})
    self.writer.close()
  ...
  def test_record_imagefile_and_masks (self):
    ...
    self.writer = DatasetWriter(out_db_file, mask_path=mask_path,
      rootdir=self.WORK_DIR)

    # Write one image.
    self.writer.addImage({'imagefile': 'images/existing1.jpg', 'mask': mask})
    self.assertTrue(op.exists(mask_path))
    masksInDir = glob(op.join(mask_path, '*.png'))
    self.assertEqual(len(masksInDir), 1)

    # Write another image.
    self.writer.addImage({'imagefile': 'images/existing2.jpg', 'mask': mask})
    self.assertTrue(op.exists(mask_path))
    masksInDir = glob(op.join(mask_path, '*.png'))
  ...
  def test_record_imagefile_maskfile_and_fields (self):
    ...
    self.writer = DatasetWriter(out_db_file, rootdir=self.WORK_DIR)
    self.writer.addImage({'imagefile': 'images/existing1.jpg', 'maskfile': 'masks/existing1.png'})
    self.writer.addImage({'imagefile': 'images/existing2.jpg', 
        'timestamp': datetime.strptime(timestamp, '%Y-%m-%d %H:%M:%S.%f'),
        'name': 'myname', 'score': 0.5})
    self.writer.close()

    self.assertTrue(op.exists(out_db_file))
    self.conn = sqlite3.connect('file:%s?mode=ro' % out_db_file, uri=True)
    c = self.conn.cursor()
    c.execute('SELECT imagefile,maskfile,width,height,timestamp,name,score FROM images')
    image_entries = c.fetchall()
  ...
  def test_record_objects (self):
    ...
    objectid1 = self.writer.addObject({
      ...
      'x1': 10, 'y1': 20, 'width': 30, 'height': 40, 'name': 'car', 'score': 0.5
    })
    ...
    self.assertEqual(objectField(object_entries[0], 'imagefile'), 'myimagefile1')
  ...
  def test_failed_record_imagefile (self):
    ...
    with self.assertRaises(ValueError):  # Cant use FileExistsError in Python 2.
  ...
  def test_failed_record_maskfile (self):
    ...
    with self.assertRaises(ValueError):  # Cant use FileExistsError in Python 2.
      self.writer.addImage({'image': image, 'maskfile': 'nonexisting.png'})
  ...
  def test_overwrite_video (self):
    ...
    self.writer = DatasetWriter(out_db_file1, image_path=out_video_path, media='video')
    self.writer.addImage({'image': np.zeros((100, 100, 3), dtype=np.uint8)})
    self.writer.close()
    with self.assertRaises(ValueError):  # Cant use FileExistsError in Python 2.
      self.writer = DatasetWriter(out_db_file2, image_path=out_video_path, media='video')
    self.writer = DatasetWriter(out_db_file3, image_path=out_video_path, media='video', overwrite=True)
    self.writer.close()
  ...
  def test_overwrite_images (self):
    ...
    with self.assertRaises(ValueError):  # Cant use FileExistsError in Python 2.
      self.writer = DatasetWriter(out_db_file2, image_path=out_image_path)
    self.writer = DatasetWriter(out_db_file3, image_path=out_image_path, overwrite=True)
