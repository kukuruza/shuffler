...
def examineImages(c, args):
    ...
    while True:  # Until a user hits the key for the "exit" action.
        ...
        imname = backend_db.imageField(image_entry, 'name')
        imscore = backend_db.imageField(image_entry, 'score')
...
def _findPressedObject(x, y, cars):
    for i in range(len(cars)):
        roi = backend_db.objectField(cars[i], 'roi')
...
def labelMatches(c, args):
    ...
    while action != 'exit' and index_image < len(image_entries):
        ...
        for object_ in objects1:
            util.drawScoredRoi(img1, backend_db.objectField(object_, 'roi'))
        for object_ in objects2:
            util.drawScoredRoi(img2, backend_db.objectField(object_, 'roi'))
        ...
        while action is None:
                ...
                for j1 in range(len(objects1)):
                    ...
                    for j2 in range(len(objects2)):
                        ...
                        c.execute(
                            ...
                            (backend_db.objectField(object1, 'objectid'),
                             backend_db.objectField(object2, 'objectid')))
                        matches = c.fetchall()
                        if len(matches) > 0:
            ...
            if i1 is not None and i2 is not None:
                ...
                if i1 in matchesOf1 or i2 in matchesOf2:
                    ...
                    objectid1 = backend_db.objectField(objects1[i1],
                                                       'objectid')
                    objectid2 = backend_db.objectField(objects2[i2],
                                                       'objectid')
